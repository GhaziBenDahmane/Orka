...
function Workloads(props: WorkloadProps) {
  ...
  if (props.selectedService) return <ServiceDetail service={props.selectedService} projects={props.projects} canWrite={canWriteEnvironment} canAdmin={roleRank(props.environmentRole) >= roleRank("admin")} canManageCredentials={props.canManageCredentials} canManageTags={props.canManageTags} close={() => { props.setSelectedService(null); void props.reloadServices(); }} moved={target => { props.setSelectedService(null); props.setProjectId(target.projectId); props.setEnvironmentId(target.id); props.flash(`Service moved to ${target.name}`); }} setError={props.setError} flash={props.flash} />;
  return <>
...
function ServiceDetail({ service, projects, canWrite, canAdmin, canManageCredentials, canManageTags, close, moved, setError, flash }: { service: Service; projects: Project[]; canWrite: boolean; canAdmin: boolean; canManageCredentials: boolean; canManageTags: boolean; close: () => void; moved: (target: Environment) => void; setError: (s: string) => void; flash: (s: string) => void }) {
  ...
  const [moveTargets, setMoveTargets] = useState<Environment[]>([]); const [moveEnvironmentId, setMoveEnvironmentId] = useState("");
  ...
  useEffect(() => { if (canWrite) void api.allEnvironments().then(result => setMoveTargets(result.items)).catch(reason => setError(message(reason))); }, [canWrite, setError]);
  ...
  async function moveService() { const target = moveTargets.find(environment => environment.id === moveEnvironmentId); if (!target || !window.confirm(`Move ${item.name} to ${target.name}? The Swarm stack keeps running under its existing identity.`)) return; setBusy(true); try { await api.moveService(item.id, target.id); moved(target); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
  const currentEnvironment = moveTargets.find(environment => environment.id === item.environmentId);
  const compatibleMoveTargets = moveTargets.filter(environment => environment.id !== item.environmentId && (environment.clusterId ?? "") === (currentEnvironment?.clusterId ?? ""));
  return <><button className="back" onClick={close}>← All services</button><section className="detail-title"><div className="service-icon large">{item.name.slice(0, 2).toUpperCase()}</div><div><h2>{item.name}</h2><p className="muted">{item.slug} · revision {item.revision}{templateInstance ? ` · ${templateInstance.templateKey}@${templateInstance.templateVersion}` : ""}</p></div><Status value={item.desiredState || "running"} />{canWrite && item.desiredState === "stopped" && <button className="primary push" onClick={() => action("start")} disabled={busy}>Start service</button>}{canWrite && item.desiredState !== "stopped" && <><button onClick={() => { if (window.confirm("Stop this service? Named volumes will be preserved.")) void action("stop"); }} disabled={busy}>Stop service</button><button className="primary" onClick={() => action("deploy")} disabled={busy}>Deploy revision</button></>}</section><div className="detail-grid"><section className="card editor"><div className="card-head"><h3>Compose definition</h3>{canWrite && <button onClick={() => action("save")} disabled={busy}>Save revision</button>}</div><textarea aria-label="Compose YAML" value={compose} readOnly={!canWrite} onChange={e => setCompose(e.target.value)} spellCheck={false} /></section><section className="card"><div className="card-head"><h3>Deployments</h3><button onClick={() => refresh()}>Refresh</button></div><div className="timeline">{deployments.length ? deployments.map(d => <article key={d.id}><i className={d.status} /><div><strong>Revision {d.revision}</strong><small>{new Date(d.createdAt).toLocaleString()} · {d.trigger}</small>{d.error && <p className="error">{d.error}</p>}</div><Status value={d.status} /></article>) : <p className="muted">No deployments yet.</p>}</div></section></div>
    ...
    <ServiceVariablesPanel service={item} canWrite={canWrite} refresh={refresh} flash={flash} setError={setError} />
    {canWrite && <section className="card template-origin"><div><p className="eyebrow">Organization</p><h3>Move service</h3><p className="muted">Move this record between projects without interrupting its existing stack. Only environments on the same Swarm are shown.</p></div><div className="template-upgrade"><label>Destination<select value={moveEnvironmentId} onChange={event => setMoveEnvironmentId(event.target.value)}><option value="">Select environment</option>{projects.map(project => { const environments = compatibleMoveTargets.filter(environment => environment.projectId === project.id); return environments.length ? <optgroup key={project.id} label={project.name}>{environments.map(environment => <option key={environment.id} value={environment.id}>{environment.name}</option>)}</optgroup> : null; })}</select></label><button type="button" disabled={busy || !moveEnvironmentId} onClick={() => void moveService()}>Move</button></div></section>}
...
function ServiceVariablesPanel({ service, canWrite, refresh, flash, setError }: { service: Service; canWrite: boolean; refresh: () => Promise<void>; flash: (s: string) => void; setError: (s: string) => void }) {
  const [items, setItems] = useState<{ name: string }[]>([]); const [name, setName] = useState(""); const [value, setValue] = useState(""); const [busy, setBusy] = useState(false);
  const load = useCallback(async () => { const result = await api.serviceVariables(service.id); setItems(result.items); }, [service.id]);
  useEffect(() => { void load().catch(reason => setError(message(reason))); }, [load, setError]);
  async function save(event: FormEvent) { event.preventDefault(); setBusy(true); try { await api.putServiceVariables(service.id, { [name]: value }); setName(""); setValue(""); await load(); await refresh(); flash("Variable stored; deploy the new revision to apply it"); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
  async function remove(variable: string) { if (!window.confirm(`Delete ${variable}? The running stack is unchanged until the next deployment.`)) return; setBusy(true); try { await api.deleteServiceVariable(service.id, variable); await load(); await refresh(); flash("Variable deleted; deploy the new revision to apply it"); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
  return <section className="card variable-panel"><div className="card-head"><div><p className="eyebrow">Write-only configuration</p><h3>Runtime variables</h3><p className="muted">Stored values are encrypted and never returned. Entering an existing name rotates its value.</p></div></div>{canWrite && <form onSubmit={save}><label>Name<input value={name} onChange={event => setName(event.target.value)} required maxLength={128} pattern="[A-Za-z_][A-Za-z0-9_]*" autoCapitalize="none" spellCheck={false} /></label><label>New value<input type="password" value={value} onChange={event => setValue(event.target.value)} maxLength={65536} autoComplete="new-password" spellCheck={false} /></label><button className="primary" disabled={busy}>{busy ? "Saving…" : items.some(item => item.name === name) ? "Rotate value" : "Add variable"}</button></form>}<div className="variable-list">{items.map(item => <article key={item.name}><code>{item.name}</code><span>value hidden</span>{canWrite && <button type="button" className="danger-button" disabled={busy} onClick={() => void remove(item.name)}>Delete</button>}</article>)}{!items.length && <p className="muted">No runtime variables configured.</p>}</div></section>;
}
...
function RoutePanel({ service, routes, canWrite, refresh, flash, setError }: { service: Service; routes: Route[]; canWrite: boolean; refresh: () => Promise<void>; flash: (s: string) => void; setError: (s: string) => void }) {
  const [draft, setDraft] = useState<RouteInput>(emptyRoute); const [editingId, setEditingId] = useState(""); const [busy, setBusy] = useState(false); const [authUsers, setAuthUsers] = useState<RouteBasicAuthUser[]>([]); const [certificates, setCertificates] = useState<CustomTLSCertificate[]>([]); const [authUsername, setAuthUsername] = useState(""); const [authPassword, setAuthPassword] = useState("");
  const refreshAuth = useCallback(async () => setAuthUsers((await api.routeBasicAuthUsers(service.id)).items), [service.id]);
  useEffect(() => { void Promise.all([refreshAuth(), api.customTLSCertificates().then(result => setCertificates(result.items))]).catch(reason => setError(message(reason))); }, [refreshAuth, setError]);
  function edit(item: Route) { setEditingId(item.id); setDraft({ serviceName: item.serviceName, host: item.host, pathPrefix: item.pathPrefix, internalPath: item.internalPath, stripPath: item.stripPath, enabled: item.enabled, redirectRegex: item.redirectRegex, redirectReplacement: item.redirectReplacement, redirectPermanent: item.redirectPermanent, targetPort: item.targetPort, tls: item.tls, certificateResolver: item.certificateResolver, customCertificateId: item.customCertificateId }); }
...
function VolumeProtection({ service, canWrite, canAdmin, flash, setError }: { service: Service; canWrite: boolean; canAdmin: boolean; flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  async function restore(backup: VolumeBackup) {
    const offline = service.desiredState === "stopped";
    const targetNodeId = volumes.find(volume => volume.name === backup.volumeName)?.storageNodeId ?? service.storageNodeId;
    const confirmation = window.prompt(offline
      ? `Restore ${backup.volumeName} onto assigned node ${targetNodeId}? The service remains stopped and existing volume contents are replaced. Type ${service.slug} to continue.`
      : `Restore ${backup.volumeName}? This stops every service mounting the volume and replaces its contents. Type ${service.slug} to continue.`);
    if (confirmation !== service.slug) { if (confirmation !== null) setError("Restore confirmation did not match the service slug"); return; }
    await run(() => api.restoreVolumeBackup(backup.id, confirmation, offline), offline ? "Offline volume restore queued" : "Volume restore queued");
  }
  async function rebindStorageNode() {
    if (service.desiredState !== "stopped") { setError("Stop the service and wait for the stop operation to succeed before relocating its storage assignment"); return; }
    const nodeId = window.prompt(`Enter the target Swarm node ID. Rebinding does not copy data: copy every named volume first, or use Restore offline on each retained backup after rebinding.`)?.trim();
    if (!nodeId) return;
    if (!window.confirm(`Orka will only change the placement assignment to ${nodeId}. Confirm that you will not start ${service.name} until every named volume has been copied or restored offline.`)) return;
    const confirmation = window.prompt(`Type ${service.slug} to update the storage-node assignment.`);
    if (confirmation !== service.slug) { if (confirmation !== null) setError("Storage relocation confirmation did not match the service slug"); return; }
    await run(() => api.rebindServiceStorageNode(service.id, nodeId, confirmation), "Storage node assignment updated");
  }
...
function Governance({ principal, projects, projectId, environments, environmentId, flash, setError }: { principal: Principal; projects: Project[]; projectId: string; environments: Environment[]; environmentId: string; flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  return <div className="settings-grid">
    ...
    <section className="card settings-card"><p className="eyebrow">Authentication policy</p><h2>Mandatory SSO</h2><p className="muted">Require interactive users to authenticate through an enabled OIDC or SAML provider. Existing local sessions are revoked except for the owner break-glass account.</p>{principal.role !== "owner" && <p className="muted">Only an organization owner can change this identity trust policy.</p>}<label className="check"><input type="checkbox" checked={requireSso} disabled={principal.role !== "owner"} onChange={e => setRequireSso(e.target.checked)} /> Require SSO for this organization</label><button className="primary spaced-button" disabled={busy || principal.role !== "owner"} onClick={() => { setBusy(true); api.putAuthSettings(requireSso).then(() => flash("Authentication policy saved")).catch(reason => setError(message(reason))).finally(() => setBusy(false)); }}>Save authentication policy</button></section>
...
function AIAudits({ flash, setError, navigate }: { flash: (s: string) => void; setError: (s: string) => void; navigate: (view: View) => void }) {
  const [runs, setRuns] = useState<AIAuditRun[]>([]); const [accounts, setAccounts] = useState<ServiceAccount[]>([]); const [currentFindings, setCurrentFindings] = useState<AIAuditFinding[]>([]); const [findings, setFindings] = useState<AIAuditFinding[]>([]); const [selected, setSelected] = useState(""); const [disposition, setDisposition] = useState("active"); const [severity, setSeverity] = useState(""); const [name, setName] = useState("platform-auditors"); const [days, setDays] = useState(90); const [token, setToken] = useState(""); const [busy, setBusy] = useState(false);
  const refresh = useCallback(async () => { const [runResult, accountResult, findingResult] = await Promise.all([api.aiAuditRuns(), api.serviceAccounts(), api.currentAIAuditFindings(disposition, severity)]); setRuns(runResult.items); setAccounts(accountResult.items.filter(item => item.role === "auditor")); setCurrentFindings(findingResult.items); }, [disposition, severity]);
  useEffect(() => { void refresh().catch(reason => setError(message(reason))); }, [refresh, setError]);
  ...
  async function triage(finding: AIAuditFinding, nextDisposition: AIAuditFinding["disposition"]) { const note = window.prompt(`Optional operator note for ${nextDisposition}`, finding.triageNote ?? ""); if (note === null) return; setBusy(true); try { const updated = await api.updateAIAuditFinding(finding.id, nextDisposition, note); setFindings(items => items.map(item => item.id === updated.id ? updated : item)); await refresh(); flash(`Finding marked ${nextDisposition}`); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
  function findingCard(finding: AIAuditFinding, includeAgent: boolean) { const hasEvidence = finding.evidence && Object.keys(finding.evidence).length > 0; const credentialFinding = finding.resourceType === "source_credential" || finding.resourceType === "backup_destination"; return <article key={finding.id}><Status value={finding.severity} /><div><strong>{finding.title}</strong><p>{finding.description}</p><small>{includeAgent ? `${finding.agentName} · ` : ""}{finding.category}{finding.resourceType ? ` · ${finding.resourceType}:${finding.resourceId}` : ""}{finding.occurrenceNumber > 1 ? ` · seen ${finding.occurrenceNumber} times` : ""}</small>{finding.remediation && <p className="remediation">Remediation: {finding.remediation}</p>}{hasEvidence && <details className="finding-evidence"><summary>Evidence</summary><pre>{JSON.stringify(finding.evidence, null, 2)}</pre></details>}{finding.triageNote && <p className="muted">Operator note: {finding.triageNote}</p>}<div className="actions"><Status value={finding.disposition} />{credentialFinding && <button disabled={busy} onClick={() => navigate("settings")}>Open credentials</button>}{finding.disposition === "open" && <button disabled={busy} onClick={() => void triage(finding, "acknowledged")}>Acknowledge</button>}{finding.disposition !== "resolved" && <button disabled={busy} onClick={() => void triage(finding, "resolved")}>Resolve</button>}{finding.disposition !== "open" && <button disabled={busy} onClick={() => void triage(finding, "open")}>Reopen</button>}</div></div></article>; }
  return <>
    ...
    <section className="card finding-list spaced"><div className="card-head"><div><p className="eyebrow">Latest per fingerprint</p><h2>Current findings</h2></div><span className="count">{currentFindings.length} {disposition || "total"}</span></div><div className="finding-filters"><label>Disposition<select value={disposition} onChange={event => setDisposition(event.target.value)}><option value="active">Active</option><option value="open">Open</option><option value="acknowledged">Acknowledged</option><option value="resolved">Resolved</option><option value="">All</option></select></label><label>Severity<select value={severity} onChange={event => setSeverity(event.target.value)}><option value="">All severities</option><option value="info">Info</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></label></div>{currentFindings.map(finding => findingCard(finding, true))}{!currentFindings.length && <p className="muted">No latest findings match these filters.</p>}</section>
    ...
    {selected && <section className="card finding-list"><div className="card-head"><h3>Findings</h3><span className="count">{findings.length} total</span></div>{findings.map(finding => findingCard(finding, false))}{!findings.length && <p className="muted">This run reported no findings.</p>}</section>}
  </>;
}
...
const notificationEvents = ["deployment.failed", "service.stop.failed", "service.schedule.failed", "backup.failed", "restore.failed", "restore.drill.failed", "database.migration.failed", "network.provision.failed", "network.delete.failed", "audit.archive.failed", "ai.audit.failed", "ai.finding.critical"];
...
const emptyCustomTLSCertificate = { name: "", certificatePem: "", privateKeyPem: "" };

function CustomTLSCertificatePanel({ flash, setError }: { flash: (s: string) => void; setError: (s: string) => void }) {
  const [items, setItems] = useState<CustomTLSCertificate[]>([]);
  const [input, setInput] = useState(emptyCustomTLSCertificate);
  const [editingId, setEditingId] = useState("");
  const [busy, setBusy] = useState(false);
  const refresh = useCallback(async () => setItems((await api.customTLSCertificates()).items), []);
  useEffect(() => { void refresh().catch(reason => setError(message(reason))); }, [refresh, setError]);
  function clear() { setEditingId(""); setInput(emptyCustomTLSCertificate); }
  async function save(event: FormEvent) {
    event.preventDefault(); setBusy(true);
    try {
      if (editingId) {
        const current = items.find(item => item.id === editingId);
        if (!current) throw new Error("The certificate no longer exists");
        await api.updateCustomTLSCertificate(editingId, { ...input, revision: current.revision });
      } else await api.createCustomTLSCertificate(input);
      await refresh(); flash(editingId ? "Custom TLS certificate rotated" : "Custom TLS certificate encrypted and saved"); clear();
    } catch (reason) { setError(message(reason)); } finally { setBusy(false); }
  }
  async function remove(item: CustomTLSCertificate) {
    if (!window.confirm(`Delete ${item.name}? Certificates attached to routes cannot be deleted.`)) return;
    setBusy(true); try { await api.deleteCustomTLSCertificate(item.id); await refresh(); flash("Custom TLS certificate deleted"); if (editingId === item.id) clear(); }
    catch (reason) { setError(message(reason)); } finally { setBusy(false); }
  }
  return <section className="card settings-card"><p className="eyebrow">Edge encryption</p><h2>Custom TLS certificates</h2><p className="muted">Certificate chains and private keys are encrypted at rest and never returned. Rotation updates every attached Traefik edge before the next workload deployment.</p><form onSubmit={save}>
    <label>Name<input value={input.name} onChange={event => setInput({ ...input, name: event.target.value })} required maxLength={100} /></label>
    <label>Certificate chain PEM<textarea className="compact-code" value={input.certificatePem} onChange={event => setInput({ ...input, certificatePem: event.target.value })} required spellCheck={false} autoComplete="off" /></label>
    <label>Private key PEM<textarea className="compact-code" value={input.privateKeyPem} onChange={event => setInput({ ...input, privateKeyPem: event.target.value })} required spellCheck={false} autoComplete="off" /></label>
    {editingId && <p className="muted">Enter the complete replacement chain and matching private key. Stored PEM material is write-only.</p>}
    <div className="actions"><button className="primary" disabled={busy}>{editingId ? "Rotate certificate" : "Add certificate"}</button>{editingId && <button type="button" disabled={busy} onClick={clear}>Cancel rotation</button>}</div>
  </form><div className="admin-items">{items.map(item => <article key={item.id}><div><strong>{item.name}</strong><small>{item.dnsNames.join(", ") || item.commonName} · expires {new Date(item.notAfter).toLocaleDateString()} · revision {item.revision}</small><small title={item.fingerprint}>{item.fingerprint.slice(0, 26)}…</small></div><div className="actions"><button type="button" disabled={busy} onClick={() => { setEditingId(item.id); setInput({ name: item.name, certificatePem: "", privateKeyPem: "" }); }}>Rotate</button><button type="button" className="danger-button" disabled={busy} onClick={() => void remove(item)}>Delete</button></div></article>)}{!items.length && <p className="muted">No custom certificates configured. Routes use their ACME resolver by default.</p>}</div></section>;
}

function Settings({ canManageIdentity, flash, setError }: { canManageIdentity: boolean; flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  return <div className="settings-grid">
    ...
    <section className="card settings-card"><p className="eyebrow">Single sign-on</p><h2>OIDC providers</h2>{canManageIdentity ? <form onSubmit={event => { event.preventDefault(); const body = { ...oidcInput, domains: domains(oidcInput.domains), scopes: domains(oidcInput.scopes) }; void run(() => editingOIDC ? api.updateOIDCProvider(editingOIDC, body) : api.createOIDCProvider(body), editingOIDC ? "OIDC provider updated" : "OIDC provider enabled"); }}>
    ...
    </form> : <p className="muted">Provider configuration is read-only for administrators. An organization owner must change identity trust.</p>}<AdminItems items={oidc.map(x => ({ id: x.id, title: x.name, detail: `${x.issuer} · ${x.defaultRole}${x.enabled ? "" : " · disabled"}`, action: x.enabled ? "Disable" : "Enable" }))} action="Disable" onEdit={canManageIdentity ? id => { const x = oidc.find(item => item.id === id); if (x) { setEditingOIDC(id); setOIDCInput({ name: x.name, issuer: x.issuer, clientId: x.clientId, clientSecret: "", domains: x.domains.join(", "), scopes: x.scopes.join(", "), defaultRole: x.defaultRole }); } } : undefined} onAction={canManageIdentity ? id => { const x = oidc.find(item => item.id === id); return run(() => x?.enabled ? api.disableOIDCProvider(id) : api.enableOIDCProvider(id), x?.enabled ? "OIDC provider disabled" : "OIDC provider enabled"); } : undefined} /></section>

    <section className="card settings-card"><p className="eyebrow">Enterprise federation</p><h2>SAML providers</h2>{canManageIdentity ? <form onSubmit={event => { event.preventDefault(); const body = { ...samlInput, domains: domains(samlInput.domains) }; void run(() => editingSAML ? api.updateSAMLProvider(editingSAML, body) : api.createSAMLProvider(body), editingSAML ? "SAML provider updated" : "SAML provider enabled"); }}>
...
function AdminItems({ items, action, onAction, onEdit }: { items: { id: string; title: string; detail: string; action?: string }[]; action: string; onAction?: (id: string) => Promise<unknown>; onEdit?: (id: string) => void }) {
  return <div className="admin-items">{items.map(item => <article key={item.id}><div><strong>{item.title}</strong><small>{item.detail}</small></div>{(onEdit || onAction) && <div className="actions">{onEdit && <button type="button" onClick={() => onEdit(item.id)}>Edit</button>}{onAction && <button type="button" onClick={() => { const label = item.action ?? action; if (window.confirm(`${label} ${item.title}?`)) void onAction(item.id); }}>{item.action ?? action}</button>}</div>}</article>)}{!items.length && <p className="muted">None configured.</p>}</div>;
}
