...
function Templates({ environmentId, canWrite, canAdmin, reloadServices, flash, setError }: { environmentId: string; canWrite: boolean; canAdmin: boolean; reloadServices: () => Promise<void>; flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  const [repositories, setRepositories] = useState<TemplateRepository[]>([]); const [repository, setRepository] = useState({ name: "", slug: "", repositoryUrl: "https://github.com/", gitRef: "main", catalogPath: "" });
  ...
  async function addRepository(event: FormEvent) { event.preventDefault(); setBusy(true); try { await api.createTemplateRepository(repository); await reloadCatalog(); setRepository({ name: "", slug: "", repositoryUrl: "https://github.com/", gitRef: "main", catalogPath: "" }); flash("Template repository registered"); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
