...
type WorkloadProps = {
  ...
  canCreateProject: boolean; projectRole: Role | ""; environmentRole: Role | ""; canManageCredentials: boolean;
};
...
function Workloads(props: WorkloadProps) {
  ...
  if (props.selectedService) return <ServiceDetail service={props.selectedService} canWrite={canWriteEnvironment} canManageCredentials={props.canManageCredentials} close={() => { props.setSelectedService(null); void props.reloadServices(); }} setError={props.setError} flash={props.flash} />;
  return <>
...
function ServiceDetail({ service, canWrite, canManageCredentials, close, setError, flash }: { service: Service; canWrite: boolean; canManageCredentials: boolean; close: () => void; setError: (s: string) => void; flash: (s: string) => void }) {
  ...
  return <><button className="back" onClick={close}>← All services</button><section className="detail-title"><div className="service-icon large">{item.name.slice(0, 2).toUpperCase()}</div><div><h2>{item.name}</h2><p className="muted">{item.slug} · revision {item.revision}{templateInstance ? ` · ${templateInstance.templateKey}@${templateInstance.templateVersion}` : ""}</p></div><Status value={item.status || "configured"} />{canWrite && <button className="primary push" onClick={() => action("deploy")} disabled={busy}>Deploy revision</button>}</section><div className="detail-grid"><section className="card editor"><div className="card-head"><h3>Compose definition</h3>{canWrite && <button onClick={() => action("save")} disabled={busy}>Save revision</button>}</div><textarea aria-label="Compose YAML" value={compose} readOnly={!canWrite} onChange={e => setCompose(e.target.value)} spellCheck={false} /></section><section className="card"><div className="card-head"><h3>Deployments</h3><button onClick={() => refresh()}>Refresh</button></div><div className="timeline">{deployments.length ? deployments.map(d => <article key={d.id}><i className={d.status} /><div><strong>Revision {d.revision}</strong><small>{new Date(d.createdAt).toLocaleString()} · {d.trigger}</small>{d.error && <p className="error">{d.error}</p>}</div><Status value={d.status} /></article>) : <p className="muted">No deployments yet.</p>}</div></section></div>
    ...
    </div></form> : <section className="card source-editor"><div className="card-head"><div><h3>Application build</h3><p className="muted">Read-only access. A developer grant is required to change this source.</p></div></div><code>{source ? `${source.buildType} · ${source.repositoryUrl || source.artifact?.filename || "uploaded source"}` : "No application source configured."}</code></section>}<section className="card logs"><div className="card-head"><h3>Service logs</h3><button onClick={() => action("logs")}>Load logs</button></div><pre>{logs || "Logs are loaded on demand to avoid unnecessary manager traffic."}</pre></section></>;
}
...
function Templates({ environmentId, canWrite, canAdmin, reloadServices, flash, setError }: { environmentId: string; canWrite: boolean; canAdmin: boolean; reloadServices: () => Promise<void>; flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  const emptyRepository = { name: "", slug: "", repositoryUrl: "https://github.com/", gitRef: "main", catalogPath: "", trustedPublicKey: "", requireSignature: true, syncIntervalSeconds: 3600 };
  const [repositories, setRepositories] = useState<TemplateRepository[]>([]); const [repository, setRepository] = useState(emptyRepository);
  const [selected, setSelected] = useState<Template | null>(null); const [name, setName] = useState(""); const [baseDomain, setBaseDomain] = useState(""); const [values, setValues] = useState<Record<string, string>>({}); const [preview, setPreview] = useState<TemplatePreview | null>(null); const [busy, setBusy] = useState(false);
  const reloadCatalog = useCallback(async () => { const [templates, sources] = await Promise.all([api.templates(), canAdmin ? api.templateRepositories() : Promise.resolve({ items: [] })]); setItems(templates.items); setRepositories(sources.items); }, [canAdmin]);
  useEffect(() => { void reloadCatalog().catch(reason => setError(message(reason))); }, [reloadCatalog, setError]);
  function open(template: Template) { if (!environmentId) { setError("Select an environment under Workloads first"); return; } setSelected(template); setName(template.name); setBaseDomain(""); setPreview(null); setValues(Object.fromEntries(template.variables.filter(variable => variable.default !== undefined).map(variable => [variable.name, variable.default ?? ""]))); }
  ...
  async function syncRepository(id: string) { setBusy(true); try { const result = await api.syncTemplateRepository(id); await reloadCatalog(); flash(`Imported ${result.imported} templates`); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
...
function AIAudits({ flash, setError }: { flash: (s: string) => void; setError: (s: string) => void }) {
  const [runs, setRuns] = useState<AIAuditRun[]>([]); const [findings, setFindings] = useState<AIAuditFinding[]>([]); const [selected, setSelected] = useState(""); const [name, setName] = useState("platform-auditors"); const [days, setDays] = useState(90); const [token, setToken] = useState(""); const [busy, setBusy] = useState(false);
  const refresh = useCallback(async () => setRuns((await api.aiAuditRuns()).items), []);
  useEffect(() => { void refresh().catch(reason => setError(message(reason))); }, [refresh, setError]);
  async function createIdentity(event: FormEvent) { event.preventDefault(); setBusy(true); try { const result = await api.createAuditorAccount(name, days); setToken(result.token); flash("Auditor identity created"); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } }
  ...
  return <>
    ...
    {selected && <section className="card finding-list"><div className="card-head"><h3>Findings</h3><span className="count">{findings.length} total</span></div>{findings.map(finding => <article key={finding.id}><Status value={finding.severity} /><div><strong>{finding.title}</strong><p>{finding.description}</p><small>{finding.category}{finding.resourceType ? ` · ${finding.resourceType}:${finding.resourceId}` : ""}</small>{finding.remediation && <p className="remediation">Remediation: {finding.remediation}</p>}</div></article>)}{!findings.length && <p className="muted">This run reported no findings.</p>}</section>}
  </>;
}
...
function Settings({ flash, setError }: { flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  return <div className="settings-grid">
    ...
    </form><AdminItems items={saml.map(x => ({ id: x.id, title: x.name, detail: `${x.domains.join(", ")} · ${x.defaultRole}${x.enabled ? "" : " · disabled"}`, action: x.enabled ? "Disable" : "Enable" }))} action="Disable" onEdit={id => { const x = saml.find(item => item.id === id); if (x) { setEditingSAML(id); setSAMLInput({ name: x.name, metadataXml: "", domains: x.domains.join(", "), emailAttribute: x.emailAttribute, nameAttribute: x.nameAttribute, defaultRole: x.defaultRole, allowIdpInitiated: x.allowIdpInitiated }); } }} onAction={id => { const x = saml.find(item => item.id === id); return run(() => x?.enabled ? api.disableSAMLProvider(id) : api.enableSAMLProvider(id), x?.enabled ? "SAML provider disabled" : "SAML provider enabled"); }} /></section>
  </div>;
}
