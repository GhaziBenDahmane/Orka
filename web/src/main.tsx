...
function AIAudits({ flash, setError }: { flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  function findingCard(finding: AIAuditFinding, includeAgent: boolean) { const hasEvidence = finding.evidence && Object.keys(finding.evidence).length > 0; return <article key={finding.id}><Status value={finding.severity} /><div><strong>{finding.title}</strong><p>{finding.description}</p><small>{includeAgent ? `${finding.agentName} · ` : ""}{finding.category}{finding.resourceType ? ` · ${finding.resourceType}:${finding.resourceId}` : ""}{finding.occurrenceNumber > 1 ? ` · seen ${finding.occurrenceNumber} times` : ""}</small>{finding.remediation && <p className="remediation">Remediation: {finding.remediation}</p>}{hasEvidence && <details className="finding-evidence"><summary>Evidence</summary><pre>{JSON.stringify(finding.evidence, null, 2)}</pre></details>}{finding.triageNote && <p className="muted">Operator note: {finding.triageNote}</p>}<div className="actions"><Status value={finding.disposition} />{finding.disposition === "open" && <button disabled={busy} onClick={() => void triage(finding, "acknowledged")}>Acknowledge</button>}{finding.disposition !== "resolved" && <button disabled={busy} onClick={() => void triage(finding, "resolved")}>Resolve</button>}{finding.disposition !== "open" && <button disabled={busy} onClick={() => void triage(finding, "open")}>Reopen</button>}</div></div></article>; }
  return <>
