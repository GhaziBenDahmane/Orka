...
function Clusters({ flash, setError }: { flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  const refresh = useCallback(async () => {
    const [clusters, history] = await Promise.all([api.clusters(), api.agentUpgrades()]);
...
function AIAudits({ flash, setError }: { flash: (s: string) => void; setError: (s: string) => void }) {
  const [runs, setRuns] = useState<AIAuditRun[]>([]); const [accounts, setAccounts] = useState<ServiceAccount[]>([]); const [findings, setFindings] = useState<AIAuditFinding[]>([]); const [selected, setSelected] = useState(""); const [name, setName] = useState("platform-auditors"); const [days, setDays] = useState(90); const [token, setToken] = useState(""); const [busy, setBusy] = useState(false);
  const refresh = useCallback(async () => { const [runResult, accountResult] = await Promise.all([api.aiAuditRuns(), api.serviceAccounts()]); setRuns(runResult.items); setAccounts(accountResult.items.filter(item => item.role === "auditor")); }, []);
...
const notificationEvents = ["deployment.failed", "backup.failed", "restore.failed", "restore.drill.failed", "database.migration.failed", "audit.archive.failed", "ai.audit.failed"];
...
function Settings({ flash, setError }: { flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  return <div className="settings-grid">
    ...
    </form><AdminItems items={saml.map(x => ({ id: x.id, title: x.name, detail: `${x.domains.join(", ")} · ${x.defaultRole}${x.enabled ? "" : " · disabled"}${x.certificateConfigurationOk && x.idpCertificateNotAfter && x.spCertificateNotAfter ? ` · IdP until ${new Date(x.idpCertificateNotAfter).toLocaleDateString()} · SP until ${new Date(x.spCertificateNotAfter).toLocaleDateString()}` : " · certificate attention required"}`, action: x.enabled ? "Disable" : "Enable" }))} action="Disable" onEdit={id => { const x = saml.find(item => item.id === id); if (x) { setEditingSAML(id); setSAMLInput({ name: x.name, metadataXml: "", domains: x.domains.join(", "), emailAttribute: x.emailAttribute, nameAttribute: x.nameAttribute, defaultRole: x.defaultRole, allowIdpInitiated: x.allowIdpInitiated }); } }} onAction={id => { const x = saml.find(item => item.id === id); return run(() => x?.enabled ? api.disableSAMLProvider(id) : api.enableSAMLProvider(id), x?.enabled ? "SAML provider disabled" : "SAML provider enabled"); }} /></section>
  </div>;
}
