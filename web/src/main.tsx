...
type SourceDraft = {
  repositoryUrl: string; gitRef: string; contextDirectory: string; dockerfile: string; buildType: "dockerfile" | "static" | "nixpacks"; outputDirectory: string; buildTarget: string;
...
function ServiceDetail({ service, close, setError, flash }: { service: Service; close: () => void; setError: (s: string) => void; flash: (s: string) => void }) {
  ...
  async function saveSource(event: FormEvent) {
    ...
    try {
      ...
      const buildSecrets = sourceForm.buildType !== "dockerfile" || sourceForm.clearBuildSecrets ? {} : sourceForm.buildSecrets.trim() ? parseBuildValues(sourceForm.buildSecrets, "Build secrets") : undefined;
