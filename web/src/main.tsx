...
const emptySourceDraft: SourceDraft = { repositoryUrl: "", gitRef: "main", contextDirectory: ".", dockerfile: "Dockerfile", buildTarget: "", enableSubmodules: false, buildArguments: "", buildSecrets: "", targetService: "web", registryImage: "", clearBuildArguments: false, clearBuildSecrets: false };

function sourceDraft(source: ApplicationSource | null): SourceDraft {
  return source ? { repositoryUrl: source.repositoryUrl, gitRef: source.gitRef, contextDirectory: source.contextDirectory, dockerfile: source.dockerfile, buildTarget: source.buildTarget ?? "", enableSubmodules: source.enableSubmodules, buildArguments: "", buildSecrets: "", targetService: source.targetService, registryImage: source.registryImage, clearBuildArguments: false, clearBuildSecrets: false } : { ...emptySourceDraft };
}
...
function ServiceDetail({ service, close, setError, flash }: { service: Service; close: () => void; setError: (s: string) => void; flash: (s: string) => void }) {
  ...
  const refresh = useCallback(async () => { const [detail, history] = await Promise.all([api.service(service.id), api.deployments(service.id)]); setItem(detail.service); setCompose(detail.service.composeYaml ?? ""); setSource(detail.source); setSourceForm(sourceDraft(detail.source)); setDeployments(history.items); }, [service.id]);
  ...
  return <><button className="back" onClick={close}>← All services</button><section className="detail-title"><div className="service-icon large">{item.name.slice(0, 2).toUpperCase()}</div><div><h2>{item.name}</h2><p className="muted">{item.slug} · revision {item.revision}</p></div><Status value={item.status || "configured"} /><button className="primary push" onClick={() => action("deploy")} disabled={busy}>Deploy revision</button></section><div className="detail-grid"><section className="card editor"><div className="card-head"><h3>Compose definition</h3><button onClick={() => action("save")} disabled={busy}>Save revision</button></div><textarea aria-label="Compose YAML" value={compose} onChange={e => setCompose(e.target.value)} spellCheck={false} /></section><section className="card"><div className="card-head"><h3>Deployments</h3><button onClick={() => refresh()}>Refresh</button></div><div className="timeline">{deployments.length ? deployments.map(d => <article key={d.id}><i className={d.status} /><div><strong>Revision {d.revision}</strong><small>{new Date(d.createdAt).toLocaleString()} · {d.trigger}</small>{d.error && <p className="error">{d.error}</p>}</div><Status value={d.status} /></article>) : <p className="muted">No deployments yet.</p>}</div></section></div>
    <form className="card source-editor" onSubmit={saveSource}><div className="card-head"><div><h3>Git and Dockerfile build</h3><p className="muted">Build an immutable image, then deploy it through the Compose service.</p></div><button className="primary" disabled={busy}>{busy ? "Saving…" : "Save source"}</button></div><div className="source-fields"><label className="wide">Repository URL<input type="url" value={sourceForm.repositoryUrl} onChange={e => setSourceForm({ ...sourceForm, repositoryUrl: e.target.value })} placeholder="https://github.com/acme/app.git" required /></label><label>Git ref<input value={sourceForm.gitRef} onChange={e => setSourceForm({ ...sourceForm, gitRef: e.target.value })} placeholder="main" /></label><label>Context directory<input value={sourceForm.contextDirectory} onChange={e => setSourceForm({ ...sourceForm, contextDirectory: e.target.value })} placeholder="." /></label><label>Dockerfile<input value={sourceForm.dockerfile} onChange={e => setSourceForm({ ...sourceForm, dockerfile: e.target.value })} placeholder="Dockerfile" /></label><label>Target stage<input value={sourceForm.buildTarget} onChange={e => setSourceForm({ ...sourceForm, buildTarget: e.target.value })} placeholder="runtime (optional)" /></label><label>Compose target service<input value={sourceForm.targetService} onChange={e => setSourceForm({ ...sourceForm, targetService: e.target.value })} placeholder="web" required /></label><label className="wide">Registry image<input value={sourceForm.registryImage} onChange={e => setSourceForm({ ...sourceForm, registryImage: e.target.value })} placeholder="registry.example.com/team/app" required /></label><label className="check wide"><input type="checkbox" checked={sourceForm.enableSubmodules} onChange={e => setSourceForm({ ...sourceForm, enableSubmodules: e.target.checked })} /> Clone same-origin Git submodules recursively</label>
