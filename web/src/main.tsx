...
function Workloads(props: WorkloadProps) {
  ...
  if (props.selectedService) return <ServiceDetail service={props.selectedService} canWrite={canWriteEnvironment} canAdmin={roleRank(props.environmentRole) >= roleRank("admin")} canManageCredentials={props.canManageCredentials} close={() => { props.setSelectedService(null); void props.reloadServices(); }} setError={props.setError} flash={props.flash} />;
  return <>
...
function ServiceDetail({ service, canWrite, canAdmin, canManageCredentials, close, setError, flash }: { service: Service; canWrite: boolean; canAdmin: boolean; canManageCredentials: boolean; close: () => void; setError: (s: string) => void; flash: (s: string) => void }) {
