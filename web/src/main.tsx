...
function VolumeProtection({ service, canWrite, canAdmin, flash, setError }: { service: Service; canWrite: boolean; canAdmin: boolean; flash: (s: string) => void; setError: (s: string) => void }) {
  ...
  async function rebindStorageNode() {
    ...
    const nodeId = window.prompt(`Enter the target Swarm node ID. Orka does not copy volume data: every named volume must already be copied or restored to that node.`)?.trim();
    if (!nodeId) return;
    if (!window.confirm(`Confirm that every named volume for ${service.name} has already been copied or restored to node ${nodeId}. Orka will only change the placement assignment.`)) return;
