...
import { api, APIError, ApplicationSource, AuditArchive, AuditEvent, BackupDestination, Cluster, Database, DatabaseBackup, DatabaseMigration, DatabaseRestore, Deployment, Environment, NotificationEndpoint, OIDCProvider, Principal, Project, ResourcePolicy, SAMLProvider, Service, SourceCredential, session, Template } from "./api";
import "./styles.css";
...
function Console({ principal, onLogout }: { principal: Principal; onLogout: () => void }) {
  ...
  return <div className="shell">
    ...
    <main className="content">
      ...
      {view === "databases" && <Databases environmentId={environmentId} canWrite={(["developer", "admin", "owner"] as string[]).includes(principal.role)} canAdmin={(["admin", "owner"] as string[]).includes(principal.role)} flash={flash} setError={setError} />}
...
function Databases({ environmentId, canWrite, canAdmin, flash, setError }: { environmentId: string; canWrite: boolean; canAdmin: boolean; flash: (s: string) => void; setError: (s: string) => void }) {
