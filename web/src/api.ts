...
export type Service = { id: string; environmentId: string; name: string; slug: string; stackName?: string; storageNodeId?: string; revision: number; desiredState: "running" | "stopped"; status: string; composeYaml?: string };
export type Route = { id: string; composeServiceId: string; serviceName: string; host: string; pathPrefix: string; internalPath: string; stripPath: boolean; enabled: boolean; redirectRegex: string; redirectReplacement: string; redirectPermanent: boolean; targetPort: number; tls: boolean; certificateResolver: string; createdAt: string; updatedAt: string };
export type RouteInput = Pick<Route, "serviceName" | "host" | "pathPrefix" | "internalPath" | "stripPath" | "enabled" | "redirectRegex" | "redirectReplacement" | "redirectPermanent" | "targetPort" | "tls" | "certificateResolver">;
