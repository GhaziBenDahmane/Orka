...
export type TemplateRepository = { id: string; name: string; slug: string; repositoryUrl: string; gitRef: string; catalogPath: string; trustedPublicKey?: string; requireSignature: boolean; enabled: boolean; lastSyncStatus: string; lastSyncError?: string; lastSyncedAt?: string };
...
export const api = {
  ...
  createTemplateRepository: (body: { name: string; slug: string; repositoryUrl: string; gitRef: string; catalogPath: string; trustedPublicKey: string; requireSignature: boolean }) => request<TemplateRepository>("/v1/template-repositories", { method: "POST", body: JSON.stringify(body) }),
