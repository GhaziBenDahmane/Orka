...
export type Cluster = { id: string; name: string; slug: string; state: string; labels: Record<string, unknown>; capacity: Record<string, unknown>; agentVersion: string; agentImage: string; agentUpdateState: string; dockerVersion: string; certificateAuthorityFingerprint?: string; pendingCertificateAuthorityFingerprint?: string; certificateNotAfter?: string; lastSeenAt?: string; maintenanceStartsAt?: string; maintenanceEndsAt?: string };
...
export type Database = { id: string; environmentId: string; composeServiceId: string; name: string; slug: string; engine: string; version: string; driverSource: "built-in" | "external" | "unbound"; driverArtifactDigest?: string; status: string };
...
export const api = {
  ...
  service: (serviceId: string) => request<{ service: Service; routes: unknown[]; source: ApplicationSource | null; template: TemplateInstance | null; reconciliation: ServiceReconciliation | null }>(`/v1/services/${serviceId}`),
