...
export type Deployment = { id: string; revision: number; status: string; trigger: string; commitSha?: string; error?: string; output?: string; createdAt: string; startedAt?: string; finishedAt?: string };
...
export type WebhookIntegration = { id: string; composeServiceId: string; name: string; provider: "github" | "gitlab" | "gitea" | "bitbucket"; branch: string; enabled: boolean; createdAt: string; updatedAt: string };
...
export type OIDCProvider = { id: string; revision: number; name: string; issuer: string; clientId: string; domains: string[]; scopes: string[]; defaultRole: string; enabled: boolean };
export type SAMLProvider = { id: string; revision: number; name: string; domains: string[]; emailAttribute: string; nameAttribute: string; defaultRole: string; allowIdpInitiated: boolean; enabled: boolean; spCertificateNotAfter?: string; idpCertificateNotAfter?: string; certificateConfigurationOk: boolean; pendingCertificateNotAfter?: string; pendingCertificateCreatedAt?: string };
export type Database = { id: string; environmentId: string; composeServiceId: string; name: string; slug: string; engine: string; version: string; driverSource: "built-in" | "external" | "unbound"; driverArtifactDigest?: string; managementKind: "managed" | "compose"; connectionServiceName?: string; storageNodeId?: string; config: Record<string, unknown>; status: string };
export type LinkedDatabaseInput = { name: string; engine: string; version: string; connectionServiceName: string; database?: string; username?: string; password: string; port: number };
export type DatabaseEngine = { name: string; defaultVersion: string; source: "built-in" | "external"; artifactDigest?: string; backupCapable: boolean; backupExtension: string; persistentConfigKeys?: string[] };
export type DatabaseMigration = { id: string; databaseInstanceId: string; sourceKind: string; sourceId: string; sourceEngine: string; sourceVersion: string; sourceHost: string; status: string; sizeBytes?: number; sha256?: string; output?: string; sourceUtilityImage?: string; targetUtilityImage?: string; readinessImage?: string; error?: string; createdAt: string; startedAt?: string; finishedAt?: string };
...
export type DatabaseBackup = { id: string; databaseInstanceId: string; status: string; artifactValid: boolean; format: string; sizeBytes?: number; sha256?: string; encrypted: boolean; destinationId?: string; utilityImage?: string; error?: string; createdAt: string; startedAt?: string; finishedAt?: string };
export type DatabaseRestore = { id: string; databaseBackupId: string; status: string; kind: string; utilityImage?: string; readinessImage?: string; error?: string; createdAt: string; startedAt?: string; finishedAt?: string };
...
export type VolumeBackup = { id: string; composeServiceId: string; volumeName: string; storageNodeId: string; destinationId: string; quiesce: boolean; status: string; artifactValid: boolean; sizeBytes?: number; sha256?: string; plaintextSha256?: string; error?: string; createdAt: string; startedAt?: string; finishedAt?: string };
...
export const api = {
  ...
  deleteProject: (projectId: string) => request<{ status: "deletion_queued" }>(`/v1/projects/${projectId}`, { method: "DELETE" }),
  ...
  deleteEnvironment: (environmentId: string) => request<{ status: "deletion_queued" }>(`/v1/environments/${environmentId}`, { method: "DELETE" }),
  ...
  deleteService: (serviceId: string, deleteVolumes: boolean) => request<{ status: "deletion_queued" }>(`/v1/services/${serviceId}?deleteVolumes=${deleteVolumes}`, { method: "DELETE" }),
  ...
  rollbackService: (serviceId: string) => request<Deployment>(`/v1/services/${serviceId}/rollback`, { method: "POST", body: "{}" }),
  cancelDeployment: (deploymentId: string) => request<{ status: "cancellation_requested" }>(`/v1/deployments/${deploymentId}/cancel`, { method: "POST", body: "{}" }),
  ...
  webhookIntegrations: (serviceId: string) => request<Envelope<WebhookIntegration>>(`/v1/services/${serviceId}/webhooks`),
