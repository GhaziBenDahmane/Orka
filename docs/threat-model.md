...
|---|---|---|---|
| Public API and console | session/service-account hashes, RBAC policy | HTTP bodies, identifiers, forwarded metadata, server responses consumed by operator tools | authentication, tenant-scoped queries, bounded request/response handling, trusted-proxy-aware per-client and per-credential rate limits, security headers, non-cacheable API responses |
