...
| SSO and SCIM | provider configuration, SP keys, SCIM token hashes | discovery documents, assertions, claims, directory writes | exact issuer/audience/domain checks, HTTPS-only issuers, redirect-free OIDC requests, PKCE/nonce/state, XML signatures, replay protection, tenant binding |
