...
|---|---|---|---|
| Public API and console | session/service-account hashes, RBAC policy | HTTP bodies, identifiers, forwarded metadata, server responses consumed by operator tools | authentication, tenant-scoped queries, bounded request/response handling, trusted-proxy-aware per-client and pre-lookup per-identifier rate limits for authentication and webhook entry points, constant-work missing-account password checks, security headers, non-cacheable API responses |
...
| SSO and SCIM | provider configuration, SP keys, SCIM token hashes | discovery documents, assertions, claims, directory writes | exact issuer/audience/domain checks, HTTPS-only OIDC and SAML endpoints, redirect-free and response-bounded OIDC requests, PKCE/nonce/state, XML signatures, replay protection, tenant binding, pre-authentication client and credential rate limits |
