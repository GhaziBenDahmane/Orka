...
Traefik labels, and local Compose secret/config imports. Stack-owned overlays
are encrypted. Safe Compose input cannot declare or attach the shared platform
routing network; the compiler attaches it only to services with tenant-scoped
approved routes. The controller and remote agent retain Docker-manager
authority and therefore remain high-value trusted components; tenant workloads
never receive their sockets or credentials.
