...
credential or audit mutation. The exact service-account token that authenticated
an identity-administration request is also revalidated, so rotating that token
fences requests still waiting to mutate identity state.
...
`network.delete.failed`, `template.sync.failed`, `audit.archive.failed`, `ai.audit.failed`, and
...
| GET | `/v1/ai/audit-runs/self` | List only the authenticating auditor identity's runs for post-deployment verification |
...
The first five endpoints require an `auditor` service account; the last four
...
Prometheus exposes backup-policy state, automatic restore-verification state,
overdue backup, and latest successful drill duration signals per database. A
backup is overdue after twice its configured interval, with a
