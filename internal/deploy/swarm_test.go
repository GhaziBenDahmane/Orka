...
`
	...
	for _, expected := range []string{"service create --quiet --detach", "node.id==nodeabc123", "type=volume,source=stack_data,target=/volume", "registry.example/dockyard@sha256:" + digest, "volume-artifact"} {
