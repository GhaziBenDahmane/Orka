...
func (s *Store) ListDatabaseRestores(ctx context.Context, organizationID, databaseInstanceID uuid.UUID) ([]DatabaseRestore, error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&item.ID, &item.DatabaseBackupID, &item.Status, &item.Kind, &item.UtilityImage, &item.ReadinessImage, &item.Error, &item.CreatedAt, &item.StartedAt, &item.FinishedAt); err != nil {
			return nil, err
		}
		items = append(items, item)
	}
