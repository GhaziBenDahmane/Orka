...
func (s *Store) GetCommitStatusDelivery(ctx context.Context, id uuid.UUID) (CommitStatusDelivery, error) {
	var item CommitStatusDelivery
	err := s.Pool.QueryRow(ctx, `UPDATE commit_status_deliveries cs SET status='running',started_at=COALESCE(started_at,now())
