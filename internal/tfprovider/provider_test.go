...
func TestProviderMetadataSchemaAndResources(t *testing.T) {
	...
	if !slices.Contains(resourceTypes, "dockyard_deploy_token") {
		t.Fatalf("provider resource types = %v", resourceTypes)
	}
	...
	if !slices.Contains(resourceTypes, "dockyard_project_tags") {
		t.Fatalf("provider resource types = %v", resourceTypes)
	}
	if !slices.Contains(resourceTypes, "dockyard_network") || !slices.Contains(resourceTypes, "dockyard_service_networks") {
		t.Fatalf("provider resource types = %v", resourceTypes)
	}
}
