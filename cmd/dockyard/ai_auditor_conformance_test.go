...
func TestAIAuditorEndToEndConformance(t *testing.T) {
	...
	statements := []struct {
		...
		{`INSERT INTO jobs(id,kind,payload,resource_key,status,created_at,run_after) VALUES($1,'backup.volume',$2,$3,'pending',now()-interval '20 minutes',now()-interval '20 minutes')`, []any{uuid.New(), `{"secret":"` + secretMarker + `"}`, "service:" + serviceID.String()}},
		...
		{`INSERT INTO application_sources(compose_service_id,repository_url,target_service,registry_image,registry_credential_id,created_at,updated_at) VALUES($1,$2,'app','registry.example.test/conformance/app',$3,now()-interval '3 minutes',now()-interval '3 minutes')`, []any{serviceID, "https://example.test/" + secretMarker + "/conformance.git", overdueSourceCredentialID}},
	...
	model := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		requiredSnapshotFragments := []string{
			...
			`"sourceBuildPosture"`, `"sourceType":"git"`, `"repositoryTransport":"https"`, `"gitRefPinned":false`,
			`"currentSourceDeployed":true`, `"deploymentCommitRecorded":false`,
	...
	evidence, _ := json.Marshal(map[string]any{
		...
		"sourceBuildProvenanceAudited":      true,
