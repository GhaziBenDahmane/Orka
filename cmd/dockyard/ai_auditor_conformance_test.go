...
func TestAIAuditorEndToEndConformance(t *testing.T) {
	...
	templateRepositoryID := uuid.New()
	volumeBackupPolicyID, offlineVolumeBackupID, offlineVolumeRestoreID := uuid.New(), uuid.New(), uuid.New()
	...
	statements := []struct {
		...
		{`INSERT INTO notification_deliveries(id,endpoint_id,event_type,resource_type,resource_id,payload,status,last_error,finished_at) VALUES($1,$2,'deployment.failed','deployment','failed-deployment',$3,'failed',$4,now()-interval '10 minutes')`, []any{failedNotificationDeliveryID, notificationEndpointID, `{"secret":"` + secretMarker + `"}`, "notification-error:" + secretMarker}},
		...
		{`INSERT INTO clusters(id,organization_id,name,slug,state,capacity,capabilities,certificate_ca_fingerprint,certificate_not_after,last_seen_at) VALUES($1,$2,'Legacy CA cluster','legacy-ca','active',$3,$4,$5,now()+interval '1 day',now())`, []any{clusterID, organizationID, `{"nodes":3,"readyNodes":2,"activeNodes":1,"schedulableNodes":0,"managers":0,"nanoCpus":2000000000,"memoryBytes":4000000000,"secret":"` + secretMarker + `"}`, `{"protocolVersion":1,"dockerSwarm":false,"dockerCompose":false,"edgeProxy":{"provider":"traefik","managementMode":"external","serviceName":"` + secretMarker + `","publicNetwork":"` + secretMarker + `","dynamicConfigurationMode":"file","dynamicConfigurationPath":"/` + secretMarker + `","ready":false,"status":"network_missing","supportsCustomCertificates":false}}`, previousAgentCAFingerprint}},
		{`INSERT INTO environments(id,project_id,cluster_id,name,slug,minimum_nodes,minimum_nano_cpus,minimum_memory_bytes) VALUES($1,$2,$3,'Production','production',2,4000000000,8000000000)`, []any{environmentID, projectID, clusterID}},
		{`INSERT INTO compose_services(id,environment_id,name,slug,stack_name,storage_node_id,compose_yaml,encrypted_env,revision) VALUES($1,$2,'Sensitive service','sensitive-service',$3,'nodeabc123',$4,$5,2)`, []any{serviceID, environmentID, "ai-conformance-" + serviceID.String(), "services: {app: {image: example.invalid/private, environment: [" + secretMarker + "], volumes: [uploads:/data]}}\nvolumes: {uploads: {}}", "encrypted:" + secretMarker}},
		...
		{`INSERT INTO template_repositories(id,organization_id,name,slug,repository_url,git_ref,enabled,last_sync_status,last_sync_error,encrypted_webhook_secret,sync_interval_seconds) VALUES($1,$2,'Unsigned failing catalog','unsigned-failing-catalog','https://github.com/example/private-catalog','main',true,'failed',$3,$4,3600)`, []any{templateRepositoryID, organizationID, "template-sync-error:" + secretMarker, "template-webhook:" + secretMarker}},
		...
		{`INSERT INTO volume_backup_policies(id,compose_service_id,volume_name,destination_id,interval_seconds,retention_count,quiesce,enabled,next_run_at) VALUES($1,$2,'uploads',$3,86400,7,true,true,now()+interval '1 day')`, []any{volumeBackupPolicyID, serviceID, overdueBackupDestinationID}},
		{`INSERT INTO volume_backups(id,volume_backup_policy_id,compose_service_id,volume_name,storage_node_id,destination_id,quiesce,status,object_key,size_bytes,sha256,plaintext_sha256,encrypted_data_key,finished_at) VALUES($1,$2,$3,'uploads','oldnode',$4,true,'succeeded','offline-recovery.enc',42,$5,$6,'encrypted',now()-interval '2 hours')`, []any{offlineVolumeBackupID, volumeBackupPolicyID, serviceID, overdueBackupDestinationID, strings.Repeat("a", 64), strings.Repeat("b", 64)}},
		...
		{`INSERT INTO backup_policies(id,database_instance_id,interval_seconds,retention_count,enabled,verify_restore,next_run_at,destination_id) VALUES($1,$2,86400,7,true,true,now()+interval '1 day',$3)`, []any{uuid.New(), failedDatabaseID, overdueBackupDestinationID}},
	}
