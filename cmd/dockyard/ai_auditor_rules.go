...
func deterministicAuditFindings(snapshot store.AIAuditSnapshot, now time.Time) []modelFinding {
	...
	if snapshot.DeployTokenPosture.UnusedActiveTokensOlderThan30Days > 0 {
		evidence := map[string]any{"unusedActiveTokens": snapshot.DeployTokenPosture.UnusedActiveTokens, "unusedActiveTokensOlderThan30Days": snapshot.DeployTokenPosture.UnusedActiveTokensOlderThan30Days}
		if snapshot.DeployTokenPosture.OldestUnusedActiveTokenCreatedAt != nil {
			evidence["oldestUnusedCreatedAt"] = snapshot.DeployTokenPosture.OldestUnusedActiveTokenCreatedAt.UTC().Format(time.RFC3339)
		}
	...
	if finalizers.FailedJobs > 0 || finalizers.ResourcesWithoutActiveJob > 0 {
		add(modelFinding{Severity: "high", Category: "operations", Title: "Resource deletion finalizer requires intervention", Description: "One or more deleting resources have a failed finalizer or no pending/running finalizer job.", ResourceType: "organization", ResourceID: snapshot.Organization.String(), Evidence: map[string]any{"deletingProjects": finalizers.DeletingProjects, "deletingEnvironments": finalizers.DeletingEnvironments, "deletingServices": finalizers.DeletingServices, "deletingDatabases": finalizers.DeletingDatabases, "deletingClusters": finalizers.DeletingClusters, "pendingJobs": finalizers.PendingJobs, "runningJobs": finalizers.RunningJobs, "failedJobs": finalizers.FailedJobs, "resourcesWithoutActiveJob": finalizers.ResourcesWithoutActiveJob}, Remediation: "Inspect the failed deletion jobs and Swarm state, restore worker access, then retry or safely reconstruct the missing finalizer job."})
	} else if finalizers.OldestRequestedAt != nil && now.Sub(*finalizers.OldestRequestedAt) > finalizerStallThreshold {
		add(modelFinding{Severity: "medium", Category: "operations", Title: "Resource deletion finalizer is stalled", Description: "A resource has remained in deletion for more than fifteen minutes while its finalizer is still pending or running.", ResourceType: "organization", ResourceID: snapshot.Organization.String(), Evidence: map[string]any{"deletingProjects": finalizers.DeletingProjects, "deletingEnvironments": finalizers.DeletingEnvironments, "deletingServices": finalizers.DeletingServices, "deletingDatabases": finalizers.DeletingDatabases, "deletingClusters": finalizers.DeletingClusters, "pendingJobs": finalizers.PendingJobs, "runningJobs": finalizers.RunningJobs, "oldestRequestedAt": finalizers.OldestRequestedAt.UTC().Format(time.RFC3339)}, Remediation: "Inspect worker and Swarm availability, then confirm the finalizer completes before retrying dependent deletions."})
	}
