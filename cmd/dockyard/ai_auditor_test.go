...
func fullyCoveredNotifications() []store.AIAuditNotificationPosture {
	return []store.AIAuditNotificationPosture{{Enabled: true, Events: []string{
		"deployment.failed", "service.stop.failed", "service.schedule.failed", "backup.failed", "restore.failed", "restore.drill.failed", "database.migration.failed", "network.provision.failed", "network.delete.failed", "audit.archive.failed", "ai.audit.failed", "ai.finding.critical",
	}}}
}
