	...
	platform := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/v1/ai/audit-snapshot":
			_ = json.NewEncoder(w).Encode(map[string]any{
				...
				"notificationPosture": []map[string]any{{"enabled": true, "events": []string{"deployment.failed", "service.stop.failed", "service.schedule.failed", "backup.failed", "restore.failed", "restore.drill.failed", "database.migration.failed", "network.provision.failed", "network.delete.failed", "template.sync.failed", "agent.upgrade.failed", "audit.archive.failed", "ai.audit.failed", "ai.finding.critical"}}},
			})
		case r.URL.Path == "/v1/ai/audit-runs":
			w.WriteHeader(http.StatusCreated)
			_ = json.NewEncoder(w).Encode(map[string]string{"id": "00000000-0000-0000-0000-000000000001"})
...
func fullyCoveredNotifications() []store.AIAuditNotificationPosture {
	return []store.AIAuditNotificationPosture{{Enabled: true, Events: []string{
		"deployment.failed", "service.stop.failed", "service.schedule.failed", "backup.failed", "restore.failed", "restore.drill.failed", "database.migration.failed", "network.provision.failed", "network.delete.failed", "template.sync.failed", "agent.upgrade.failed", "audit.archive.failed", "ai.audit.failed", "ai.finding.critical",
	}}}
}
