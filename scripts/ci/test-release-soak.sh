...
database_engines="$(curl --fail-with-body --silent --show-error "${auth_headers[@]}" "$base_url/v1/database-engines")"
jq -e '
  .engines as $engines |
  ([$engines[].name] | sort) == ["clickhouse","libsql","mariadb","meilisearch","mongo","mysql","postgres","qdrant","redis","timescaledb","valkey"] and
  ($engines | all(.source == "built-in" and (.artifactDigest | test("^sha256:[a-f0-9]{64}$")))) and
  ([$engines[].artifactDigest] | unique | length) == 11
' <<<"$database_engines" >/dev/null || fail "released controller exposes an incomplete or unbound built-in database driver inventory"
