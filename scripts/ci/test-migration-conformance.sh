...
evidence_file="${DOCKYARD_MIGRATION_EVIDENCE:-migration-conformance.json}"
postgres_image="${DOCKYARD_MIGRATION_POSTGRES_IMAGE:-postgres@sha256:742f40ea20b9ff2ff31db5458d127452988a2164df9e17441e191f3b72252193}"
run_id="$$-${RANDOM}"
postgres_container="dockyard-migration-postgres-${run_id}"
work_dir=$(mktemp -d)
...
for command in docker go jq; do
  command -v "$command" >/dev/null || { echo "$command is required for migration conformance" >&2; exit 1; }
done

cd "$root_dir"
go test -c -o "$work_dir/migrate.test" ./internal/migrate
...
export DOCKYARD_MIGRATION_CONFORMANCE=1

"$work_dir/migrate.test" \
  -test.timeout=2m \
  -test.run='^TestImportDokployDryRunAndIdempotence$' \
  -test.count=1 -test.v | tee "$work_dir/conformance.log"

grep -F -- '--- PASS: TestImportDokployDryRunAndIdempotence ' "$work_dir/conformance.log" >/dev/null
sed -n 's/^MIGRATION_EVIDENCE //p' "$work_dir/conformance.log" >"$work_dir/evidence.json"
test "$(wc -l <"$work_dir/evidence.json")" -eq 1

jq \
  --arg sourceCommit "${GITHUB_SHA:-$(git rev-parse HEAD)}" \
  --arg createdAt "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  '. + {sourceCommit:$sourceCommit,createdAt:$createdAt,liveDokployCutover:"staging-required"}' \
...
jq -e '
  ...
  .applicationsImported and .routesImported and .databasesImported == 9 and
