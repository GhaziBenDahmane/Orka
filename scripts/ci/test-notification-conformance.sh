...
"$work_dir/store.test" \
  -test.timeout=2m \
  -test.run='^(TestCommitStatusFailuresUseDeploymentTenantEvent|TestCommitStatusDeliveryHistoryAndAuditedRedrive|TestDeletionFinalizerFailuresUseDedicatedTenantEvent|TestDeletionFinalizerHistoryAndAuditedRedrive|TestEdgeCertificateFailureNotificationsFanOutToAffectedTenants|TestNotificationDeliveryHistoryAndAuditedRedrive)$' \
  -test.count=1 -test.v | tee -a "$work_dir/conformance.log"

"$work_dir/httpapi.test" \
  -test.timeout=2m \
  -test.run='^(TestNotificationDeliveryHistoryAndRetryAPI|TestCommitStatusDeliveryHistoryAndRetryAPI|TestDeletionFinalizerHistoryAndRetryAPI)$' \
  -test.count=1 -test.v | tee -a "$work_dir/conformance.log"

"$work_dir/auditor.test" \
  -test.timeout=2m \
  -test.run='^(TestDeterministicAuditDetectsExhaustedNotificationDelivery|TestDeterministicAuditDetectsExhaustedCommitStatusDelivery|TestDeterministicAuditDetectsStuckFinalizers)$' \
  -test.count=1 -test.v | tee -a "$work_dir/conformance.log"
...
(
  ...
  "$work_dir/observability.test" \
    -test.timeout=2m \
    -test.run='^(TestDatabaseMetricsQueriesRemainValid|TestPrometheusAlertsCoverExhaustedNotificationDeliveries|TestPrometheusAlertsCoverExhaustedCommitStatusDeliveries|TestPrometheusAlertsCoverDeployTokensAndFinalizers)$' \
    -test.count=1 -test.v
...
grep -F -- '--- PASS: TestDeletionFinalizerFailuresUseDedicatedTenantEvent ' "$work_dir/conformance.log" >/dev/null
grep -F -- '--- PASS: TestDeletionFinalizerHistoryAndAuditedRedrive ' "$work_dir/conformance.log" >/dev/null
...
grep -F -- '--- PASS: TestDeletionFinalizerHistoryAndRetryAPI ' "$work_dir/conformance.log" >/dev/null
...
grep -F -- '--- PASS: TestDeterministicAuditDetectsStuckFinalizers ' "$work_dir/conformance.log" >/dev/null
...
grep -F -- '--- PASS: TestPrometheusAlertsCoverDeployTokensAndFinalizers ' "$work_dir/conformance.log" >/dev/null
...
jq -e '
  ...
  .deletionFinalizerFailureNotification and
  .deletionFinalizerHistoryRedacted and
  .deletionFinalizerTenantIsolation and
